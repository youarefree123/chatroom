...
//                showFileOpenDialog(frame);
        ...
        // 设置“发送”按钮监听
        listener = new Client().new ChatViewListen(); //new一个文本发送监听类
        listener.setJTextField(text);  // 获取消息文本域
        listener.setJTextArea(textArea); //获取公共聊天域
        listener.setChatViewJf(frame); //获取框架
        text.addActionListener(listener);  // 文本框添加监听：可以直接回车发送消息
